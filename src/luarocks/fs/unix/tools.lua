...
function tools:command_at(directory, cmd)
   return "cd " .. self:Q(self:absolute_name(directory)) .. " && " .. cmd
end
...
function tools:make_dir(directory)
   assert(directory)
   local ok, err = self:execute(vars.MKDIR.." -p", directory)
   if not ok then
...
function tools:remove_dir_if_empty(directory)
   assert(directory)
   self:execute_quiet(vars.RMDIR, directory)
end
...
function tools:remove_dir_tree_if_empty(directory)
   assert(directory)
   self:execute_quiet(vars.RMDIR, "-p", directory)
end
...
function tools:copy(src, dest, perm)
   assert(src and dest)
   if self:execute(vars.CP, src, dest) then
      if perm then
         if self:is_dir(dest) then
            dest = dir.path(dest, dir.base_name(src))
         end
         if self:chmod(dest, perm) then
            return true
         else
...
function tools:copy_contents(src, dest)
   assert(src and dest)
   if self:execute_quiet(vars.CP.." -pPR "..self:Q(src).."/* "..self:Q(dest)) then
      return true
   else
      return false, "Failed copying "..src.." to "..dest
   end
...
function tools:delete(arg)
   assert(arg)
   assert(arg:sub(1,1) == "/")
   self:execute_quiet(vars.RM, "-rf", arg)
end
...
function tools:find(at)
   ...
   local result = {}
   local pipe = self.io_popen(self:command_at(at, self:quiet_stderr(vars.FIND.." *")))
...
function tools:zip(zipfile, ...)
   return self:execute(vars.ZIP.." -r", zipfile, ...)
end
...
function tools:unzip(zipfile)
   assert(zipfile)
   return self:execute_quiet(vars.UNZIP, zipfile)
end
...
function tools:exists(file)
   assert(file)
   return self:execute(vars.TEST, "-e", file)
end
...
function tools:is_dir(file)
   assert(file)
   return self:execute(vars.TEST, "-d", file)
end
...
function tools:is_file(file)
   assert(file)
   return self:execute(vars.TEST, "-f", file)
end

function tools:chmod(pathname, mode)
   if mode then 
      return self:execute(vars.CHMOD, mode, pathname)
   else
      return false
   end
end
...
function tools:unpack_archive(archive)
   ...
   if not self.verbose then
      pipe_to_tar = " 2> /dev/null"..self:quiet(pipe_to_tar)
   end

   local ok
   if archive:match("%.tar%.gz$") or archive:match("%.tgz$") then
      ok = self:execute_string(vars.GUNZIP.." -c "..self:Q(archive)..pipe_to_tar)
   elseif archive:match("%.tar%.bz2$") then
      ok = self:execute_string(vars.BUNZIP2.." -c "..self:Q(archive)..pipe_to_tar)
   elseif archive:match("%.zip$") then
      ok = self:execute_quiet(vars.UNZIP, archive)
...
function tools:attributes(filename, attrtype)
   ...
   local pipe = self.io_popen(self:quiet_stderr(vars.STAT.." "..flag.." "..self:Q(filename)))
...
function tools:browser(url)
   return self:execute(cfg.web_browser, url)
end

function tools:set_time(file, time)
   file = dir.normalize(file)
   return self:execute(vars.TOUCH, "-d", "@"..tostring(time), file)
end
...
function tools:make_temp_dir(name)
   ...
   local pipe = self.io_popen(vars.MKTEMP.." -d "..self:Q(template))
