...
-- using third-party modules like LuaFileSystem, LZLib, etc.
-- module("luarocks.fs.mods")
local fs_mods = {}
...
-- The command is executed in the current directory.
-- @param cmd string: No quoting/escaping is applied to the command.
-- @return boolean: true if command succeeds (status code 0), false
-- otherwise.
function fs_mods:execute_string(cmd)
...
function fs_mods:change_dir(d)
   table.insert(self.dir_stack, lfs.currentdir())
   d = dir.normalize(d)
   return lfs.chdir(d)
end
...
-- a crossplatform way.
function fs_mods:change_dir_to_root()
   local current = lfs.currentdir()
   if not current or current == "" then
      return false
   end
   table.insert(self.dir_stack, current)
...
function fs_mods:pop_dir()
   local d = table.remove(self.dir_stack)
   if d then
      lfs.chdir(d)
      return true
   else
...
function fs_mods:make_dir(directory)
   ...
   for d in directory:gmatch("([^/]+)/*") do
      ...
      if not mode then
         ...
         ok, err = self:chmod(path, cfg.perm_exec)
         if not ok then
            return false, err
         end
...
function fs_mods:remove_dir_if_empty(d)
   assert(d)
   d = dir.normalize(d)
   lfs.rmdir(d)
end
...
-- if already does not exist)
-- @param d string: pathname of directory to remove.
function fs_mods:remove_dir_tree_if_empty(d)
   assert(d)
   d = dir.normalize(d)
   for i=1,10 do
      lfs.rmdir(d)
...
function fs_mods:copy(src, dest, perms)
   assert(src and dest)
   ...
   if not perms then perms = self:attributes(src, "permissions") end
...
-- @return boolean or (boolean, string): true on success, false on failure
local function recursive_copy(self, src, dest, perms)
   local srcmode = lfs.attributes(src, "mode")

   if srcmode == "file" then
      local ok = self:copy(src, dest, perms)
      if not ok then return false end
   elseif srcmode == "directory" then
      local subdir = dir.path(dest, dir.base_name(src))
      local ok, err = self:make_dir(subdir)
      if not ok then return nil, err end
      for file in lfs.dir(src) do
         if file ~= "." and file ~= ".." then
            local ok = recursive_copy(self, dir.path(src, file), subdir, perms)
            if not ok then return false end
         end
      end
   end
...
-- @return boolean or (boolean, string): true on success, false on failure,
-- plus an error message.
function fs_mods:copy_contents(src, dest, perms)
   assert(src and dest)
   src = dir.normalize(src)
   dest = dir.normalize(dest)
   ...
   for file in lfs.dir(src) do
      if file ~= "." and file ~= ".." then
         local ok = recursive_copy(self, dir.path(src, file), dest, perms)
         if not ok then
...
-- @return nil
function fs_mods:dir_iterator(at)
...
function fs_mods:is_file(file)
   assert(file)
   file = dir.normalize(file)
   return lfs.attributes(file, "mode") == "file"
end

function fs_mods:set_time(file, time)
   file = dir.normalize(file)
...
function fs_mods:unzip(filename)
   ...
   repeat
      if file.filename:sub(#file.filename) == "/" then
         local ok, err = self:make_dir(dir.path(self:current_dir(), file.filename))
         if not ok then return nil, err end
      else
         local base = dir.dir_name(file.filename)
         if base ~= "" then
            base = dir.path(self:current_dir(), base)
            if not self:is_dir(base) then
               local ok, err = self:make_dir(base)
               if not ok then return nil, err end
            end
         end
         ...
         rf:close()
         local wf, err = io.open(dir.path(self:current_dir(), file.filename), "wb")
...
function fs_mods:download(url, filename, cache)
   ...
   if cfg.no_proxy then
      return self:use_downloader(url, filename, cache)
   end
...
function fs_mods:get_md5(file)
   file = self:absolute_name(file)
...
function fs_mods:chmod(file, mode)
...
function fs_mods:attributes(file, attrtype)
...
function fs_mods:make_temp_dir(name)
